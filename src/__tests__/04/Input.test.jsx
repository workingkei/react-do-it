...
describe('<Input>', () => {
  ...
  describe('contains <Input>', () => {
    it('renders one input', () => {
      const wrpper = shallow(<Input name="test_name" />);
      expect(wrpper.find('input')).toHaveLength(1);
      expect(wrpper.find('label')).toHaveLength(1);
    });
    ...
    it('renders errorMessage', () => {
      ...
      expect(wrapper.find('span').prop('class')).toBe('error');
